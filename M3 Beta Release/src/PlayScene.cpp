	...
	if (m_pPlayerFire.size() > 0)
	{
		for (unsigned i = 0; i < m_pPlayerFire.size(); i++)
		{
			...
			for (SDL_Rect* r : tileLocation)
			{
				...
				{
					SoundManager::Instance().setSoundVolume(15);
...
void PlayScene::start()
{
	...
	SoundManager::Instance().load("../Assets/audio/collisionwall.wav", "collisionwall", SOUND_SFX);
	...
	SoundManager::Instance().load("../Assets/audio/enemyspawn.ogg", "enemyspawn", SOUND_SFX);
	SoundManager::Instance().load("../Assets/audio/bulletspawn.wav", "bulletspawn", SOUND_SFX);
...
void PlayScene::spawn()
{
	...
	if (randomAction == 0)
	{
		...
		SoundManager::Instance().playSound("bulletspawn", 0, -1);
	}
	else if (randomAction == 1)
	{
		...
		m_pEnemy[m_pEnemy.size() - 1]->setEnabled(true);
		addChild(m_pEnemy[m_pEnemy.size() - 1], 1, 3);
		SoundManager::Instance().setSoundVolume(17);
